...
def reorder_statements(list_to_reorder):
    ...
    # Add the data from the old list to the new stack to reorder it.
    for counter in range(len(list_to_reorder)):
        ordered_statements.append(list_to_reorder.pop())

    # Return the new stack to the main program.
...
def print_statement(list_to_display):
    for counter in range(len(list_to_display)):
        ...
        # Removes and returns the last stored value in the stack.
        item = list_to_display.pop()
